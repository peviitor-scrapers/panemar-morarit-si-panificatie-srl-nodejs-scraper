...
## Project
Panemar scraper for peviitor.ro (Node.js, ESM, Jest)

## 🌱 This Repo Is a Derived Scraper
Acest repo este derivat din template-ul [EPAM scraper](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper) al ecosistemului peviitor.ro.
...
- **Only the scraping logic in `scraper/index.js`** (`fetchCareersPage`, `parsePanemarJobs`) is Panemar-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/panemar-morarit-si-panificatie-srl-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/panemar-morarit-si-panificatie-srl-nodejs-scraper --state open`
