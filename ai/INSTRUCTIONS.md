...
This scraper extracts job listings from pagina de angajari Panemar and imports them to peviitor.ro.

Target: https://panemar.ro
...
1. **Start with brand** - We know the brand ("Panemar")
...
8. **Scrape new jobs** - Extract jobs from API-ul de angajari Panemar (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts panemar.ro jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from API-ul de angajari Panemar (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Panemar API (jobs for Romania)
    │
    ▼
...
- **API-ul de angajari Panemar**: `https://panemar.ro/angajari/` — GET with query params (country, page, size)
